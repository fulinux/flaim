...
FSTATIC FLMBOOL flmCh6Cmbcar(
	FLMUINT16 *			pui16WpChar,
	FLMUINT16			ui16BaseChar,
	FLMINT16				ui16DiacriticChar);

FSTATIC FLMUINT16 flmCh6Upper(
	FLMUINT16			ui16WpChar);

FSTATIC FLMBOOL flmIsUpper(
	FLMUINT16			ui16WpChar);

FSTATIC FLMUINT16 flmCh6Lower(
	FLMUINT16			ui16WpChar);
...
FSTATIC FLMUINT16 flmGetCollation(
	FLMUINT16			ui16WpChar,
	FLMUINT				uiLanguage);
...
Desc:	Defines the range of characters within the set which are case
		convertible
****************************************************************************/
FSTATIC FLMBYTE flmCaseConvertableRange[] =
{
...
Desc:
****************************************************************************/
FSTATIC FLMUINT16 WP_UTOWP60[ UTOWP60_ENTRIES][ 2] =
{
	{ 0x00A1, 0x0407 },					//  7 ,  4,
	{ 0x00A2, 0x0413 },					// 19 ,  4,
	{ 0x00A3, 0x040b },					// 11 ,  4,
...
Desc:
****************************************************************************/
FSTATIC BASE_DIACRIT_TABLE flm_ml1c_table[] =
{
	{ 'A',  acute },
	{ 'a',  acute },
	{ 'A',  circum },
...
Desc:
****************************************************************************/
FSTATIC BASE_DIACRIT_TABLE flm_grk_c_table[] = 
{
...
Desc:
****************************************************************************/
FSTATIC BASE_DIACRIT flm_grk_c = 
{
...
Desc:
****************************************************************************/
FSTATIC BASE_DIACRIT_TABLE flm_rus_c_table[] = 
{
...
Desc:
****************************************************************************/
FSTATIC BASE_DIACRIT flm_rus_c = {
...
{
	{F_CHSASCI, flmAsc60Tbl},
	{F_CHSMUL1, flmMn60Tbl},
	{F_CHSSYM1, flmSym60Tbl},
	{F_CHSGREK, flmGrk60Tbl},
	{F_CHSCYR,  flmCyrl60Tbl},
	{0xFF, 	 	0}
};
...
TBL_B_TO_BP flmHebArabicCol60Tbl[] = 
{
	...
	{F_CHSSYM1, 	flmSym60Tbl},
	{F_CHSGREK, 	flmGrk60Tbl},
	{F_CHSHEB,		flmHeb60TblA},
	{F_CHSHEB,		flmHeb60TblB},
	{F_CHSARB1,		flmAr160Tbl},
	{F_CHSARB2,		flmAr260Tbl},
	{0xff, 			0}
};
...
#endif
{
	...
	// Make sure the diacritics constants are defined correctly
	
	f_assert( F_GRAVE == grave);
	f_assert( F_CENTERD == centerd);
	f_assert( F_TILDE == tilde);
	f_assert( F_CIRCUM == circum);
	f_assert( F_CROSSB == crossb);
	f_assert( F_SLASH == slash);
	f_assert( F_ACUTE == acute);
	f_assert( F_UMLAUT == umlaut);
	f_assert( F_MACRON == macron);
	f_assert( F_APOSAB == aposab);
	f_assert( F_APOSBES == aposbes);
	f_assert( F_APOSBA == aposba);
	f_assert( F_RING == ring);
	f_assert( F_DOTA == dota);
	f_assert( F_DACUTE == dacute);
	f_assert( F_CEDILLA == cedilla);
	f_assert( F_OGONEK == ogonek);
	f_assert( F_CARON == caron);
	f_assert( F_STROKE == stroke);
	f_assert( F_BREVE == breve);
	f_assert( F_DOTLESI == dotlesi);
	f_assert( F_DOTLESJ == dotlesj);
	f_assert( F_GACUTE == gacute);
	f_assert( F_GDIA == gdia);
	f_assert( F_GACTDIA == gactdia);
	f_assert( F_GGRVDIA == ggrvdia);
	f_assert( F_GGRAVE == ggrave);
	f_assert( F_GCIRCM == gcircm);
	f_assert( F_GSMOOTH == gsmooth);
	f_assert( F_GROUGH == grough);
	f_assert( F_GIOTA == giota);
	f_assert( F_GSMACT == gsmact);
	f_assert( F_GRGACT == grgact);
	f_assert( F_GSMGRV == gsmgrv);
	f_assert( F_GRGGRV == grggrv);
	f_assert( F_GSMCIR == gsmcir);
	f_assert( F_GRGCIR == grgcir);
	f_assert( F_GACTIO == gactio);
	f_assert( F_GGRVIO == ggrvio);
	f_assert( F_GCIRIO == gcirio);
	f_assert( F_GSMIO == gsmio);
	f_assert( F_GRGIO == grgio);
	f_assert( F_GSMAIO == gsmaio);
	f_assert( F_GRGAIO == grgaio);
	f_assert( F_GSMGVIO == gsmgvio);
	f_assert( F_GRGGVIO == grggvio);
	f_assert( F_GSMCIO == gsmcio);
	f_assert( F_GRGCIO == grgcio);
	f_assert( F_GHPRIME == ghprime);
	f_assert( F_GLPRIME == glprime);
	f_assert( F_RACUTE == racute);
	f_assert( F_RGRAVE == rgrave);
	f_assert( F_RRTDESC == rrtdesc);
...
RCODE ftkTestText( void)
{
	...
	f_printf( "Running case conversion tests ... ");
	
	for( uiLoop = 0; uiLoop <= 0xFFFF; uiLoop++)
	{
		ui16WpChar = (FLMUINT16)uiLoop;
		
		if( flmCh6Upper( ui16WpChar) != f_wpUpper( ui16WpChar) ||
			 flmIsUpper( ui16WpChar) != f_wpIsUpper( ui16WpChar) ||
			 flmCh6Lower( ui16WpChar) != f_wpLower( ui16WpChar))
		{
			f_printf( "Case conversion failed on character 0x%04X.\n", 
				(unsigned)ui16WpChar);
			rc = RC_SET_AND_ASSERT( NE_FLM_FAILURE);
			goto Exit;
		}
	...
	f_printf( "Done.\n");
	f_printf( "Running character collation tests ... ");
	...
	for( uiLoop = 0; uiLoop <= 0xFFFF; uiLoop++)
	{
		...
		for( uiLanguage = 0; uiLanguage < FLM_LAST_LANG; uiLanguage++)
		{
			...
			{
				f_printf( "Character collation failed on character 0x%04X.\n", 
					(unsigned)ui16WpChar);
				rc = RC_SET_AND_ASSERT( NE_FLM_FAILURE);
				goto Exit;
			}
	...
	f_printf( "Done.\n");
	f_printf( "Running US character collation timing tests ... ");

	uiStartTime = FLM_GET_TIMER();

	for( uiLoop = 0; uiLoop < 100; uiLoop++)
	{
		for( uiSubloop = 0; uiSubloop <= 0xFFFF; uiSubloop++)
		{
			flmGetCollation( (FLMUINT16)uiSubloop, FLM_US_LANG);
		}
	}
	...
	for( uiLoop = 0; uiLoop < 100; uiLoop++)
	{
		for( uiSubloop = 0; uiSubloop <= 0xFFFF; uiSubloop++)
		{
			f_wpGetCollation( (FLMUINT16)uiSubloop, FLM_US_LANG);
		}
	}
	...
	for( uiLoop = 0; uiLoop <= 0xFFFF; uiLoop++)
	{
		...
		{
			f_printf( "Zenkaku to Hankaku conversion failed on character 0x%04X.\n", 
				(unsigned)ui16WpChar);
			rc = RC_SET_AND_ASSERT( NE_FLM_FAILURE);
			goto Exit;
		}
	...
	f_printf( "Done.\n");
	f_printf( "Running Hankaku to Zenkaku conversion tests ... ");
	...
	for( uiLoop = 0; uiLoop <= 0xFFFF; uiLoop++)
	{
		...
		for( uiNextWpChar = 0x0B3D; uiNextWpChar <= 0x0B3E; uiNextWpChar++)
		{
			if( (HanToZenkaku( ui16WpChar, (FLMUINT16)uiNextWpChar, &ui16Zenkaku) !=
					f_wpHanToZenkaku( ui16WpChar, (FLMUINT16)uiNextWpChar, &ui16Zenkaku2)) ||
				 ui16Zenkaku != ui16Zenkaku2) 
			{
				f_printf( "Hankaku to Zenkaku conversion failed on character 0x%04X.\n", 
					(unsigned)ui16WpChar);
				rc = RC_SET_AND_ASSERT( NE_FLM_FAILURE);
				goto Exit;
			}
	...
	f_printf( "Done.\n");
	f_printf( "Running Unicode to WP conversion tests ... ");
	
	for( uiLoop = 0; uiLoop <= 0xFFFF; uiLoop++)
	{
		uzUniChar = (FLMUINT16)uiLoop;
		bConverted = FALSE;
		
		if( uzUniChar < 127)
		{
			ui16WpChar = uzUniChar;
			bConverted = TRUE;
		}
		else
		{
			...
			FLMINT16			ui16Max;
			FLMINT16			ui16Temp;
			FLMUINT16 *		pui16Table = (FLMUINT16 *) WP_UTOWP60;
			FLMUINT16		ui16TblChr;
			...
			do
			{
				ui16Temp = (ui16Min + ui16Max) >> 1;
				ui16TblChr = *(pui16Table + (ui16Temp * 2));
				
				if( ui16TblChr < uzUniChar)
				{
					ui16Min = ui16Temp + 1;
				}
				else if( ui16TblChr > uzUniChar)
				{
					ui16Max = ui16Temp - 1;
				}
				else
				{
					ui16WpChar = *(pui16Table + (ui16Temp * 2) + 1);
					bConverted = TRUE;
					break;
				}
			} while( ui16Min <= ui16Max);
		}
		
		if( f_depricatedUnicodeToWP( uzUniChar, &ui16TmpWpChar) != bConverted ||
			 ui16WpChar != ui16TmpWpChar)
		{
			f_printf( "Conversion failed on character 0x%04X.\n", 
				(unsigned)uzUniChar);
			rc = RC_SET_AND_ASSERT( NE_FLM_FAILURE);
			goto Exit;
		}
	...
	f_printf( "Done.\n");
	f_printf( "Running character break tests ... ");
	...
	f_printf( "Done.\n");
	f_printf( "Running character combine tests ... ");
	
	for( uiLoop = 0; uiLoop <= 0x0FFF; uiLoop++)
	{
		for( uiSubloop = 0; uiSubloop < 0x0FFF; uiSubloop++)
		{
	...
	FLMUINT16 *			pui16DiacriticChar)
{
	...
	if( HI( ui16WpChar) >= F_NCHSETS ||
		 (pBaseDiacritic = flm_car60_c[HI( ui16WpChar)]) == 0)
	{
		return (TRUE);
	}
	...
	FLMUINT16	ui16WpChar)
{
	...
	else
	{
		...
		if (ucCharSet == F_CHSMUL1)
		{
			...
			if (ucChar >= flmCaseConvertableRange[(F_CHSMUL1 - 1) * 2] &&
				 ucChar <= flmCaseConvertableRange[((F_CHSMUL1 - 1) * 2) + 1])
			{
	...
	do
	{
		if (pColTbl->key == ucCharSet)
		{
			...
			if (ucCharVal >= *pucColVals)
			{
				...
				ucCharVal = ucCharVal - *pucColVals++;
