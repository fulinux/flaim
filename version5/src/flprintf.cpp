...
void flmSprintfStringFormatter(
	...
	f_va_list *			args)
{
	static FLMBYTE		pszNullPointerStr[] = "<null>";
	FLMUINT				uiOutputLen;
	FLMUINT				uiCount;
	FLMUNICODE *		pUnicode;
	FLMBYTE *			pszStr = (FLMBYTE *)f_va_arg( *args, char *);
	...
	if( ucFormatChar == 'e')
	{
		pInfo->pszDestStr +=
			f_sprintf( (char *)pInfo->pszDestStr, "%s (%04X)",
				(char *)F_DbSystem::_errorString( (RCODE)uiErrorCode),
	...
	f_va_list *		args)
{
	...
	flmSprintfParseArgs( (FLMBYTE *)pszFormat, &info, args);
	*info.pszDestStr = 0;
