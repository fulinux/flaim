	...
	FLMUINT        uiStackSize)
{
	...
	f_mutexLock( gv_XFlmSysData.pThreadMgr->m_hMutex);
	...
	if( gv_XFlmSysData.pThreadMgr->m_pThreadList)
	{
		gv_XFlmSysData.pThreadMgr->m_pThreadList->m_pPrev = this;
	}

	m_pNext = gv_XFlmSysData.pThreadMgr->m_pThreadList;
	gv_XFlmSysData.pThreadMgr->m_pThreadList = this;
	...
	{
		rc = RC_SET( NE_XFLM_COULD_NOT_START_THREAD);
		goto Exit;
	}
	...
	f_mutexUnlock( gv_XFlmSysData.pThreadMgr->m_hMutex);
...
Exit:
	...
	if( RC_BAD( rc))
	{
		...
		gv_XFlmSysData.pThreadMgr->unlinkThread( this, bManagerMutexLocked);
...
#endif
{
	...
	gv_XFlmSysData.pThreadMgr->lockMutex();
	...
	gv_XFlmSysData.pThreadMgr->unlockMutex();
	...
	gv_XFlmSysData.pThreadMgr->unlinkThread( pThread, FALSE);
...
****************************************************************************/
void F_Thread::setThreadStatusStr(
