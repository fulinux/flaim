...
//
// $Id: flbackup.cpp 3112 2006-01-19 13:12:40 -0700 (Thu, 19 Jan 2006) dsanders $
//------------------------------------------------------------------------------
	...
	IF_Backup **	ppBackup)
{
	...
	*ppBackup = NULL;
	...
	if( eTransType != XFLM_READ_TRANS && eTransType != XFLM_UPDATE_TRANS)
	{
	...
	if( RC_BAD( rc = beginTrans( eTransType, uiMaxLockWait,
		XFLM_DONT_KILL_TRANS | XFLM_DONT_POISON_CACHE, &pBackup->m_dbHdr)))
	{
		goto Exit;
	}
	...
	if( eBackupType == XFLM_INCREMENTAL_BACKUP &&
...
void F_Backup::reset( void)
{
	...
	m_eBackupType = XFLM_FULL_BACKUP;
	m_backupRc = NE_XFLM_OK;
}
	...
	FLMUINT *				puiIncSeqNum)
{
	...
	if( m_eBackupType == XFLM_INCREMENTAL_BACKUP)
	{
		if( puiIncSeqNum)
		{
	...
	f_memcpy( &pucBlkBuf[ FLM_BACKER_NEXT_INC_SERIAL_NUM],
		m_ucNextIncSerialNum, XFLM_SERIAL_NUM_SIZE);
	...
	f_memcpy( &pucBlkBuf[ uiBlkBufOffset], &m_dbHdr, sizeof( XFLM_DB_HDR));
	pDbHdr = (XFLM_DB_HDR *)(&pucBlkBuf[ uiBlkBufOffset]);
	uiBlkBufOffset += m_uiBlockSize;
	...
	if (pszPassword && *pszPassword)
	{
		...
		{
			if	(RC_BAD( rc = m_pDb->dbLock(XFLM_LOCK_EXCLUSIVE, 0, XFLM_NO_TIMEOUT)))
			{
				goto Exit;
			}
			bMustUnlock = TRUE;
		...
		f_memcpy( pDbHdr->DbKey, pucTmp, pDbHdr->ui32DbKeyLen);
		f_free(  &pucTmp);
	}
...
RCODE F_Backup::endBackup( void)
{
	...
	if( RC_BAD( rc = m_pDb->beginTrans( XFLM_UPDATE_TRANS)))
	{
		goto Exit;
	}
	bStartedTrans = TRUE;
		...
		// operation
{
			...
			else
			{
					...
					&eAction, &bOKToRetry)))
				{
						...
						eAction == XFLM_RESTORE_ACTION_CONTINUE)
					{
						...
						if( eAction == XFLM_RESTORE_ACTION_CONTINUE)
						{
	...
	rc = openDatabase( pDatabase,
		...
		pszRflDir, pszPassword, XFLM_DONT_RESUME_THREADS,
	...
	FLMBOOL *				pbOKToRetry)
{
	...
	if( (eBackupType == XFLM_INCREMENTAL_BACKUP && !bIncremental) ||
		(eBackupType == XFLM_FULL_BACKUP && bIncremental))
	{
	...
	*puiNextIncSeqNum = uiIncSeqNum;
	...
	if( bIncremental)
	{
		...
		{
			rc = RC_SET( NE_XFLM_INVALID_FILE_SEQUENCE);
			goto Exit;
		}
		...
		{
			rc = RC_SET( NE_XFLM_SERIAL_NUM_MISMATCH);
			goto Exit;
		}
	...
	f_memcpy( pDbHdr->ucIncBackupSerialNum,
			ucNextIncSerialNum, XFLM_SERIAL_NUM_SIZE);
