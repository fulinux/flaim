	...
	FLMBOOL			bTimeoutAll)
{
						...
						m_pFirstLockWaiter->uiWaitTime))))
	{
		...
		*(pLockWaiter->pRc) = RC_SET( NE_XFLM_DATABASE_LOCK_REQ_TIMEOUT);
...
RCODE ServerLockObject::Lock(
	...
	XFLM_DB_STATS *	pDbStats)				// Place to collect stats.
{
	RCODE				rc = NE_XFLM_OK;
	RCODE				TempRc;
		 ...
		 ((bExclReq) && (m_uiSharedLockCnt)))
	{
		...
		if (uiMaxWaitSecs == XFLM_NO_TIMEOUT)
		{
		...
		if (bLogEvent &&
			 gv_XFlmSysData.EventHdrs [XFLM_EVENT_LOCKS].pEventCBList)
		{
			flmDoEventCallback( XFLM_EVENT_LOCKS,
	...
	FLMUINT *		puiPriorityCount)
{
	...
	*puiPriorityCount = 0;
	...
	if (m_bExclLock)
	{
		*peCurrLockType = XFLM_LOCK_EXCLUSIVE;
		*puiThreadId = m_uiLockThreadId;
	}
	else if (m_uiSharedLockCnt)
	{
		*peCurrLockType = XFLM_LOCK_SHARED;
		*puiThreadId = 0;
	}
	else
	{
		*peCurrLockType = XFLM_LOCK_NONE;
		*puiThreadId = 0;
	}
