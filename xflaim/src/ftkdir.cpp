...
Desc:
****************************************************************************/
const char * XFLMAPI F_DirHdl::CurrentItemName( void)
{
...
Desc:
****************************************************************************/
FLMBOOL XFLMAPI F_DirHdl::CurrentItemIsDir( void)
{
	return( ((m_uiAttrib & XF_IO_FA_DIRECTORY)
						 ? TRUE
						 : FALSE));
}
...
Desc:
****************************************************************************/
FLMUINT64 XFLMAPI F_DirHdl::CurrentItemSize( void)
{
...
****************************************************************************/
RCODE XFLMAPI F_DirHdl::Next( void)
{
	...
	uiSearchAttributes =
		XF_IO_FA_NORMAL | XF_IO_FA_RDONLY | XF_IO_FA_ARCHIVE | XF_IO_FA_DIRECTORY;
...
****************************************************************************/
RCODE XFLMAPI F_DirHdl::OpenDir(
...
****************************************************************************/
RCODE XFLMAPI F_DirHdl::CreateDir(
	const char *	pszDirPath)
{
	...
	// If gv_pFileSystem->pathReduce couldn't reduce the path at all, then an
	...
	if( *pszParentDir)
	{
		...
		if( RC_OK( gv_pFileSystem->Exists( pszParentDir)))
		{
		...
		else if( RC_BAD( rc = this->CreateDir( pszParentDir)))
		{
			goto Exit;
		}
	}
	...
	{
		rc = MapWinErrorToFlaim( GetLastError(), NE_XFLM_CREATING_FILE);
	}
	...
	{
		rc = MapErrnoToFlaimErr( errno, NE_XFLM_CREATING_FILE);
	}
...
****************************************************************************/
RCODE XFLMAPI F_DirHdl::RemoveDir(
	const char *	pszDirName)
{
#if defined( FLM_WIN)
	...
	FLMUINT *			puiFoundAttrib)
{
   ...
   char *		pszWildCard = "*.*";
	...
	if( uiSearchAttrib & XF_IO_FA_NORMAL )
	{
		uiSearchAttrib |= XF_IO_FA_ARCHIVE;
	}
...
Exit:
	...
	f_memset( pFindData, 0, sizeof( F_IO_FIND_DATA));
	if( uiSearchAttrib & XF_IO_FA_DIRECTORY)
	{
		pFindData->mode_flag |= S_IFDIR;
	}

	if( uiSearchAttrib & XF_IO_FA_RDONLY)
	{
	...
	if( iRetVal != 0)
	{
		...
		{
			rc = RC_SET( NE_XFLM_IO_NO_MORE_FILES);
		}
		else
		{
	...
	*puiFoundAttrib = (FLMUINT)ReturnAttributes(
	...
	// Save the search path in the NE_XFLM_IO_FIND_DATA struct
	...
	FLMUINT *			puiFoundAttrib)
{
   ...
   *puiFoundAttrib = pFindData->findBuffer.dwFileAttributes;
	...
	if( (iRetVal =  Find2( pFindData)) != 0)
	{
		...
		return( MapErrnoToFlaimErr( errno, NE_XFLM_READING_FILE));
	}
	...
	F_IO_FIND_DATA *		pFindData)
{
    ...
    (f_strcmp( pFindData->findBuffer.cFileName, ".") == 0) ||
	 (!(pFindData->uiSearchAttrib & XF_IO_FA_DIRECTORY) &&
		(pFindData->findBuffer.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))))
	{
		if( (pFindData->findBuffer.dwFileAttributes &
			...
			((pFindData->uiSearchAttrib & XF_IO_FA_NORMAL) &&
	...
	F_IO_FIND_DATA *		DirStuff)
{
	...
	for( ;;)
	{
		...
		if ( (DirStuff->mode_flag == XF_IO_FA_NORMAL) &&
