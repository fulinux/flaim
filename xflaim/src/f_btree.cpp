	...
	)
{
	...
	if (m_bOpened)
	{
		rc = RC_SET_AND_ASSERT( NE_XFLM_BTREE_BAD_STATE);
		goto Exit;
	}

	// Verify that we are in an update transaction.
	if (pDb->m_eTransType != XFLM_UPDATE_TRANS && !pDb->m_pDatabase->m_bTempDb)
	{
		rc = RC_SET_AND_ASSERT( pDb->m_eTransType == XFLM_NO_TRANS
								? NE_XFLM_NO_TRANS_ACTIVE
								: NE_XFLM_ILLEGAL_TRANS_OP);
		goto Exit;
	}
	...
	IF_ResultSetCompare *	pCompare)
{
	...
	if (pDb->m_eTransType == XFLM_NO_TRANS && !pDatabase->m_bTempDb)
	{
	...
	IF_DeleteStatus *		ifpDeleteStatus)
{
	...
	if (m_pDb->m_eTransType != XFLM_UPDATE_TRANS && !m_bTempDb)
	{
		rc = RC_SET_AND_ASSERT( m_pDb->m_eTransType == XFLM_NO_TRANS
								? NE_XFLM_NO_TRANS_ACTIVE
								: NE_XFLM_ILLEGAL_TRANS_OP);
		goto Exit;
	}
	...
	IF_DeleteStatus *		ifpDeleteStatus)
{
	...
	{
		rc = RC_SET_AND_ASSERT( pDb->getTransType() == XFLM_NO_TRANS
								? NE_XFLM_NO_TRANS_ACTIVE
								: NE_XFLM_ILLEGAL_TRANS_OP);
		goto Exit;
	}
	...
	FLMUINT *	puiNumLevels)
{
	...
	bIsIndex = m_pLFile->eLfType == XFLM_LF_INDEX ? TRUE : FALSE;
	...
	FLMUINT *			puiOffsetIndex)
{
	...
	if( bLast)
	{
		...
		rc = findEntry( pucKey, uiKeyLen, XFLM_EXACT);

		// Should not find this entry.  If we get back anything other than
		// an NE_XFLM_NOT_FOUND, then there is a problem.
		
		if( rc != NE_XFLM_NOT_FOUND)
		{
			if( RC_OK( rc))
			{
				rc = RC_SET( NE_XFLM_NOT_UNIQUE);
			}
			goto Exit;
		}
	}
	...
	FLMUINT *			puiOffsetIndex)
{
	...
	if( bFirst || bLast)
	{
		...
		if( RC_BAD( rc = findEntry( pucKey, uiKeyLen, XFLM_EXACT, NULL,
	...
	FLMUINT *		puiOffsetIndex)
{
	...
	if (m_pDb->m_eTransType == XFLM_NO_TRANS && !m_bTempDb)
	{
		rc = RC_SET_AND_ASSERT( NE_XFLM_NO_TRANS_ACTIVE);
		goto Exit;
	}
	...
	// in the case of XFLM_FIRST, XFLM_LAST, XFLM_EXCL and possibly XFLM_INCL, 
	// we will pass back the key we actually found.

	if( uiMatch != XFLM_EXACT)
	{
	...
	)
{
		  ...
		  (m_uiBlkChangeCnt != m_pDb->m_uiBlkChangeCnt)))
	{
		...
		if( m_pSCache->m_pBlkHdr->ui64TransID != m_ui64LastBlkTransId ||
				(m_pDb->m_eTransType == XFLM_UPDATE_TRANS &&
					m_pDb->m_ui64CurrTransID == m_pSCache->m_pBlkHdr->ui64TransID))
		{
			...
			{
				rc = RC_SET(NE_XFLM_BTREE_BAD_STATE);
				goto Exit;
			}
		}
	}
	...
	FLMUINT *				puiOffsetIndex)
{
		  ...
		  (m_uiBlkChangeCnt != m_pDb->m_uiBlkChangeCnt)))
	{
					...
					m_pDb->m_ui64CurrTransID == m_pSCache->m_pBlkHdr->ui64TransID))
		{
			...
			// Doing a find with XFLM_EXCL will result in our being positioned at
			// the next entry.
			
			if( RC_BAD( rc = btLocateEntry( pucKey, uiKeyBufSize, puiKeyLen,
				XFLM_EXCL, puiDataLength)))
			{
				goto Exit;
			}
	...
	// the case of XFLM_FIRST, XFLM_LAST, XFLM_EXCL and possibly XFLM_INCL,
	// we will pass back the key we actually found.
	...
	FLMUINT *				puiKeyLen)
{
	...
	// as in the case of XFLM_FIRST, XFLM_LAST, XFLM_EXCL and 
	// possibly XFLM_INCL, we will pass back the key we actually found.
...
RCODE F_Btree::createNewLevel( void)
{
	...
	{
		rc = RC_SET_AND_ASSERT( NE_XFLM_BTREE_FULL);
		goto Exit;
	}
...
Desc:	Method to advance to the next entry.  If there are no more entries
		...
		the chain, NE_XFLM_EOF_HIT will be returned.
****************************************************************************/
...
Desc:	Method to backup the stack to the previous entry.  If there are no
		...
		more blocks in the chain, NE_XFLM_BOF_HIT will be returned.
****************************************************************************/
	...
	FLMUINT *			puiOffsetIndex)	// In/Out
{
	...
	if( uiMatch == XFLM_FIRST || uiMatch == XFLM_LAST)
	{
		uiKeyLen = 0;
	}
...
Exit:
	...
	if( RC_OK( rc) || (rc == NE_XFLM_NOT_FOUND) || (rc == NE_XFLM_EOF_HIT))
	{
		if( pStack)
		{
			m_pStack = pStack;
...
Desc:	Private method to search for a particular key in a pre-designted
		...
		binary search for it.  Note that a uiMatch of XFLM_FIRST & XFLM_LAST
	...
	FLMUINT *				puiOffsetIndex)
{
	...
	if( ((F_BTREE_BLK_HDR *)pSCache->m_pBlkHdr)->ui8BTreeFlags & BLK_IS_INDEX &&
		m_pLFile->eLfType != XFLM_LF_INDEX)
	{
		rc = RC_SET( NE_XFLM_NOT_FOUND);
		goto Exit;
	}
...
Desc:	Method to search through a BTree block to find a specific key.  If 
		...
		parameter further qualifies the search.  The XFLM_FIRST & XFLM_LAST
		values will ignore the key altogether and just return the first or last
		key respectively.  The XFLM_INCL value will return the key if found or the 
		first key following if not found.  The XFLM_EXACT will return an 
		NE_XFLM_NOT_FOUND if the key cannot be found.  XFLM_EXCL will return
	...
	FLMUINT			uiMatch)
{
	...
	if( uiMatch == XFLM_FIRST)
	{
		pStack->uiCurOffset = uiTop;
		goto Exit;
	}

	if( uiMatch == XFLM_LAST || pStack->uiKeyLen == 0)
	{
		pStack->uiCurOffset = uiBottom;
		goto Exit;
	}

	flmAssert( uiMatch == XFLM_INCL || uiMatch == XFLM_EXCL || 
				  uiMatch == XFLM_EXACT);
...
ResultGreater1:
	...
	if( iResult <= 0)
	{
		if( iResult < 0 && uiMatch != XFLM_INCL)
		{
			rc = RC_SET( NE_XFLM_NOT_FOUND);
		}
		...
		switch( eAction)
		{
			...
			case ELM_DONE:
			{
				...
				else if( bMoreToRemove)
				{
					...
					if( RC_BAD( rc = findEntry( pucSavKey, uiSavKeyLen, XFLM_EXACT)))
					{
						goto Exit;
					}
				...
				else if( pucRemainingValue && uiRemainingLen)
				{
					...
					rc = findEntry( pucSavKey, uiSavKeyLen, XFLM_EXCL);

					// We could find this entry.  If we get back anything other than
					// an NE_XFLM_EOF_HIT or NE_XFLM_OK, then there is a problem.
					
					if( rc != NE_XFLM_OK && rc != NE_XFLM_EOF_HIT &&
	...
	FLMBOOL					bTruncate)
{
	...
	if( !m_bStackSetup)
	{
		if( RC_BAD( rc = findEntry( *ppucKey, *puiKeyLen, XFLM_EXACT)))
		{
			goto Exit;
		}
	}
	...
	FLMUINT *			puiCounts)
{
	...
	rcTmp = findEntry( m_pReplaceInfo->pucParentKey,
							 m_pReplaceInfo->uiParentKeyLen, XFLM_EXACT);

	m_uiSearchLevel = uiSearchLevel;

	if ((rcTmp != NE_XFLM_OK) &&
		 (rcTmp != NE_XFLM_NOT_FOUND) &&
		 (rcTmp != NE_XFLM_EOF_HIT))
	{
		rc = RC_SET( rcTmp);
		goto Exit;
	}
	...
	FLMUINT				uiKeyLen)
{
	...
	if( !m_bStackSetup)
	{
		if( RC_BAD( rc = findEntry( pucKey, uiKeyLen, XFLM_EXACT)))
		{
			goto Exit;
		}
	}
	...
	while( !bLastElement)
	{
		...
		if( pBlkHdr->ui16NumKeys == 0)
		{
			...
			if( RC_BAD( findEntry( pucKey, uiKeyLen, XFLM_EXACT)))
			{
				goto Exit;
			}
	...
	FLMUINT32			ui32ToBlkAddr)
{
	...
	{
		// We need to update LFile to reflect the new root block address.

		F_COLLECTION *		pCollection = NULL;
		F_Database *		pDatabase= m_pDb->m_pDatabase;

		m_pLFile->uiRootBlk = ui32ToBlkAddr;

		if( m_pLFile->eLfType == XFLM_LF_COLLECTION)
		{
			// Cannot use collections in a temporary DB.

			flmAssert( !m_bTempDb);
			
			if( RC_BAD( rc = m_pDb->m_pDict->getCollection( m_pLFile->uiLfNum,
				&pCollection)))
			{
				goto Exit;
			}
		...
		rc  = pDatabase->lFileWrite( m_pDb, pCollection, m_pLFile);
		goto Exit;
	}
