...
#define INITIAL_READ_BUF_SIZE	100
	...
	if( RC_BAD( gv_pFileSystem->Open( pszFileName,
									XFLM_IO_RDONLY, &m_pFile)))
	{		
		goto Exit;
	}
	...
	while (!bEOF)
	{
		...
		if ( (RC_OK( rc) || (rc == NE_XFLM_IO_END_OF_FILE)) &&
				(uiBytesInLine > 0) )
		{
...
RCODE F_IniFile::Write()
{
	...
	if (RC_BAD( rc = gv_pFileSystem->Create( m_pszFileName,
								XFLM_IO_RDWR, &m_pFile)))
	{
		goto Exit;
	}
	...
	while (pCurLine)
	{
		if (pCurLine->pszParamName)
		{
			...
			if (RC_BAD (rc = m_pFile->Write( uiFileOffset,
													   f_strlen( pCurLine->pszParamName),
													   pCurLine->pszParamName,
													   &uiBytesWritten)))
			{
				goto Exit;
			}
			...
			if (pCurLine->pszParamValue)
			{
				...
				if (RC_BAD (rc = m_pFile->Write( uiFileOffset, 1,
														   (void *)"=", &uiBytesWritten)))
				{
					goto Exit;
				}
				...
				if (RC_BAD (rc = m_pFile->Write( uiFileOffset,
															f_strlen( pCurLine->pszParamValue),
															pCurLine->pszParamValue,
															&uiBytesWritten)))
				{
					goto Exit;
				}
		...
		if (pCurLine->pszComment)
		{
			...
			if (pCurLine->pszParamName)
			{
				if (RC_BAD (rc = m_pFile->Write( uiFileOffset, 2,
													 (void *)" #", &uiBytesWritten)))
				{
					goto Exit;
				}
		...
		// Write out a newline...
		if (RC_BAD (rc = m_pFile->Write( uiFileOffset, f_strlen( "\n"),
													(void *)"\n", &uiBytesWritten)))
		{
			goto Exit;
		}
	...
	// The contents of disk now represent the the structs in memory,
	// so reset the bModified flag
	m_bModified = FALSE;
	...
	FLMBOOL *	pbMore)
{
	...
	rc = m_pFile->Read( m_uiFileOffset, *puiBytes,
