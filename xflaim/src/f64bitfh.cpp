...
// Tabs:	3
//
//		Copyright (c) 2001,2003-2006 Novell, Inc. All Rights Reserved.
//
...
//
// $Id: f64bitfh.cpp 3111 2006-01-19 13:10:50 -0700 (Thu, 19 Jan 2006) dsanders $
//------------------------------------------------------------------------------
			...
			m_szPath, "*.64", &pDir)))
		{
			...
			*/
			...
			for( rc = pDir->Next(); !RC_BAD( rc) ; rc = pDir->Next() )
			{
				...
				(void)gv_pFileSystem->Delete( szTmpPath);
			}
	...
	const char *	pszPath)
{
	...
	if( m_bOpen)
	{
		...
		return( RC_SET_AND_ASSERT( NE_XFLM_FAILURE));
	}
	...
	if( !gv_pFileSystem->IsDir( pszPath))
	{
		...
		*/
		...
		rc = gv_pFileSystem->Delete( pszPath);
		goto Exit;
	}
	...
	const char *	pszFileExtension)		// Extension to be used on the new file.
{
	...
	} while ((rc != NE_XFLM_OK) && (uiCount++ < 20));
	...
	const char *	pszPath)
{
	...
	if( RC_BAD( gv_pFileSystem->Exists( pszPath)) ||
	...
	IF_FileHdl **	ppFileHdl)
{
	...
	*ppFileHdl = NULL;
	...
	if( !pTmpHdl)
	{
		...
		if( RC_BAD( rc = gv_pFileSystem->Open( szPath, XFLM_IO_RDWR, &pTmpHdl)))
		{
			if( rc == NE_XFLM_IO_PATH_NOT_FOUND && bGetForWrite)
			{
				if( RC_BAD( rc = gv_pFileSystem->Create( szPath,
 					XFLM_IO_RDWR, &pTmpHdl)))
				{
					goto Exit;
				}
			}
...
Desc:	This is a private method that will truncate the spill file back to
		the specified size.
****************************************************************************/
RCODE F_64BitFileHandle::Truncate(
