//------------------------------------------------------------------------------
// Desc:	Contains the methods for the FFixedBlk, FBtreeRoot, FBtreeLeaf,
//			FBtreeNonLeaf, and FBtreeBlk classes.
//
// Tabs:	3
//
...
//
// $Id: fdynbtre.cpp 3112 2006-01-19 13:12:40 -0700 (Thu, 19 Jan 2006) dsanders $
//------------------------------------------------------------------------------
...
RCODE FBtreeBlk::searchEntry(
	...
	void *		pvFoundEntry)
{
	RCODE			rc = RC_SET( NE_SFLM_NOT_FOUND);
	FLMUINT		uiLow;
	FLMUINT		uiMid;
	FLMUINT		uiHigh;
	FLMUINT		uiTblSize;
	...
	void *			pvEntry)
{
	...
	for(;;)
	{
		...
		// Only returns NE_SFLM_OK or FAILURE.
	...
	FBtreeRoot *	pNewRoot)		// New Non-leaf root
{
	...
	{
		pucEntry = ENTRY_POS( uiPos);
		if ((rc = pNewRoot->search( pucEntry)) != NE_SFLM_NOT_FOUND)
		{
	...
	FLMUINT			uiCurChildAddr)
{
	...
	for (uiPos = uiMid + 1; uiPos < uiEntryCount; uiPos++)
	{
		...
		if ((rc = pRightBlk->searchEntry( pucEntry )) != NE_SFLM_NOT_FOUND)
		{
			rc = RC_SET_AND_ASSERT( NE_SFLM_FAILURE);
			goto Exit;
		}
	...
	if ((rc = searchEntry( pucEntry )) != NE_SFLM_NOT_FOUND)
	{
		rc = RC_SET_AND_ASSERT( NE_SFLM_FAILURE);
		goto Exit;
	}
...
RCODE FBtreeRoot::readBlk(
	...
	FBlkTypes		eBlkType,			// Expected access type to read
...
****************************************************************************/
RCODE FBtreeRoot::openFile( void)
{ 
	RCODE	rc = NE_SFLM_OK;

	if (!m_pFileHdl)
	{
		rc = gv_SFlmSysData.pFileSystem->createUniqueFile( m_pszFileName,
...
****************************************************************************/
void FBtreeRoot::closeFile( void)
{
	if (m_pFileHdl)
	{
		m_pFileHdl->close();
		gv_SFlmSysData.pFileSystem->deleteFile( m_pszFileName);
		m_pFileHdl->Release();
		m_pFileHdl = NULL;
	}
}
