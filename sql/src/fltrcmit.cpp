...
//
// $Id: fltrcmit.cpp 3114 2006-01-19 13:22:45 -0700 (Thu, 19 Jan 2006) dsanders $
//------------------------------------------------------------------------------
	...
	)
{
...
#ifdef FLM_DBG_LOG
	flmDbgLogUpdate( m_pDatabase, ui64TransId, 0, 0, NE_SFLM_OK, "TCmit");
#endif

	// Flush any dirty rows
	...
	for (uiLfNum = 1, pTable = m_pDict->m_pTableTbl;
		  ...
		  uiLfNum++, pTable++)
	{
		if (pTable->uiTableNum && pTable->lfInfo.bNeedToWriteOut)
		{
			if (RC_BAD( rc = m_pDatabase->lFileWrite( this, &pTable->lfInfo)))
			{
				goto Exit;
			}
		}
	...
	if (!m_bHadUpdOper)
	{
		...
		// Even if the transaction is empty, there could be "uncommitted" rows that
		// were created, but never set to "dirty" - hence the m_bHadUpdOper flag
		// would never have been set.  So we need to call freeModifiedRows to get
		// rid of them.

		m_pDatabase->freeModifiedRows( this, ui64TransId - 1);
		goto Exit1;
	}
	...
	m_pDatabase->commitRowCache();

	// Push the IXD_FIXUP values back into the F_INDEX
	...
	if (m_pIxdFixups)
	{
		...
		while (pIxdFixup)
		{
			if ((pIndex = m_pDict->getIndex( pIxdFixup->uiIndexNum)) == NULL)
			{
			...
			if (pIndex)
			{
				pIndex->ui64LastRowIndexed = pIxdFixup->ui64LastRowIndexed;
			}
...
Exit1:
	...
	else
	{
		...
		if (m_eTransType == SFLM_UPDATE_TRANS)
		{
			...
			{
				flmTransEventCallback( SFLM_EVENT_COMMIT_TRANS, this, rc,
							ui64TransId);
			}
	...
	// to SFLM_NO_TRANS up above if abortTrans was called.
	...
	if (m_eTransType == SFLM_UPDATE_TRANS)
	{
		if (RC_BAD( rc))
		{
			...
			// m_eTransType would no longer be SFLM_UPDATE_TRANS.
		...
		else
		{
			...
			if (RC_OK( rc = pRfl->completeTransWrites( this, TRUE, FALSE)))
			{
				...
				rc = m_pDatabase->doCheckpoint( m_hWaitSem, m_pDbStats, m_pSFileHdl,
						...
						TRUE, SFLM_CP_TIME_INTERVAL_REASON,
	...
	if (m_pDbStats && eSaveTransType != SFLM_NO_TRANS)
	{
